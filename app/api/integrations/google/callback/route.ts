...
export async function GET(req: NextRequest) {
  ...
  try {
    const { accessToken, refreshToken, meta } = await exchangeCodeForToken(code);
    ...
    await prisma.integration.upsert({
      ...
      update: { accessToken, refreshToken, meta: metaJson },
      create: { userId, platform: "GOOGLE", accessToken, refreshToken, meta: metaJson },
    });
  } catch (err) {
